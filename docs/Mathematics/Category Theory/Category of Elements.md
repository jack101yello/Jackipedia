...
For a [[Set-Valued Functor|set-valued functor]] $F$ on a [[Category|category]] $C$, the category of elements of $F$, denoted $\int F$, is a category such that:
...
A covariant functor/[[Contravariant Functor|contravariant functor]] is a [[Representable Functor|representable functor]] if and only if its category of elements has an [[Initial Object|initial object]]/[[Terminal Object|terminal object]].
...
For any set-valued functor $F$, the full subcategory of $\int F$ spanned by the representations of $F$ is either empty if $F$ is not representable, or is a [[Contractible Groupoid|contractible groupoid]] if it is. This can be proven fairly easily:
